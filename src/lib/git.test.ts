...
describe("path helpers", () => {
  ...
  describe("getReposDir", () => {
    it("returns repos directory path", () => {
      expect(getReposDir("project")).toBe(join("project", "opensrc", "repos"));
    });
  });

  describe("getRepoPath", () => {
    it("returns full path for repo", () => {
      const expected = join("project", "opensrc", "repos", "github.com", "vercel", "ai");
      expect(getRepoPath("github.com/vercel/ai", "project")).toBe(expected);
    });

    it("handles different hosts", () => {
      const expected = join("project", "opensrc", "repos", "gitlab.com", "owner", "repo");
      expect(getRepoPath("gitlab.com/owner/repo", "project")).toBe(expected);
    });
  });
